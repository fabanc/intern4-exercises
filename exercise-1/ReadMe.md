...
However you will see that the page displays nothing. If you open the debugger in Chrome Developer Tools, you will even see an error message `Error: The server does not support suite glob resolution`. This is because running the suite in the browser does not support glob expression to describe the location of the unit test. Instead you need a path to an existing file.
